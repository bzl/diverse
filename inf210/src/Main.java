...
public class Main {
	...
	public static void main(String[] args) {
		...
		/*
		 * Om rippleadder er true vil variablene f� verdier til seg som simulrerer 
		 * figur 3.10 alts� rippleadder, med test tallene 14+28. 
		 * 
		...
		if (rippleadder) {
			...
			nextStateFunction = new String[][]{ // current state, input, resulting state, resulting "print"
					...
					{"3","11","3","1"}};
			/*
			 * I mitt ripple adder eksempel bruker jeg tallene 14+28, alts�
			 * 001110+011100. Da eksemplet i forelesningsnotatene har tillat
			 * seg � gj�re forenklingen som skal til for at det parses, gj�r
			 * jeg ogs� dette, s� regnestykkets input format blir da
			 * 00 10 11 11 01 00, siden vi parer de 2 bakerste bits fra 
			 * hvert tall, og beveger oss fra venstre til h�yre p� tallene.
			 * Dette gj�r fors�vidt at output svaret kommer i "feil retning".
			 * Det l�ser vi ved � manuelt snu svaret i min implementasjon.
			 * 
			 * Jeg har verifisert at maskinen fungerer. Den avslutter i 
			 * state Q1, med output 010101, snur man outputten f�r man 101010,
			 * alts� 42, alts� svaret p� 14+28 :=)
		...
		do{
			...
			if(inputData.length() == alphabetX[0].length()){ //H�nderer slutten av en iterasjon
				complete = true;
				currentData = inputData;
			} else {
				currentData  = inputData.substring(0, alphabetX[0].length()); 
				inputData = inputData.substring(alphabetX[0].length(), inputData.length()); //beskj�rer input (i stedefor � lage en ny variabel som lagrer v�r posisjon).
			}

			for(int i = 0; i < nextStateFunction.length; i++){ //iterer igjennom alle regler for � finne den aktuelle for denne iterasjonen
				if(Integer.parseInt(nextStateFunction[i][0]) == currentXState){
					if(nextStateFunction[i][1].equals(currentData)){
						...
						break; // Stop n�r vi har funnet aktuel regel, og lest/endret n�dvendigt.
					}
				}
			}
